...
class PrismAction(object):
    ...
    def addStateToSource(self,pvar):
        if type(pvar) is list:
            for i in range(len(pvar)):
                ppvar = pvar[i]
                self.addStateToSource(ppvar)
        elif type(pvar) is PrismVariable:
            self.src.append(pvar)
        else:
            raise Exception("Invalid State addition "+str(pvar))
        ...
        if type(pvar) is list:
            for i in range(len(pvar)):
                ...
                if type(prob) is list:
                    ...
                    pprob = prob
                self.addStateToDestination(ppvar,pprob)
        elif type(pvar) is PrismVariable:
            self.dest.append(self.createDestinationDict(pvar,prob))
        else:
            raise Exception("Invalid State addition in dest "+str(pvar))
