...
class GeneratePrismFile(object):
    ...
    def addRewardNameToPFR(self,pfr,rewName):
    ...
    #steps to create the drone stuff
    #we start with a single drone
    #we just generate the x and y commands
    #and thats it

    def createSingleXYModule(self,pfr,modName,modVarNames,startIndex,endIndex,modRewName,startIndexC,endIndexC,probValC,nameSuffix):
        gmod = PrismModule(modName,[],pfr.constants)
        for modVarName in modVarNames:    
            gmod.addVariable(modVarName,startIndex,endIndex,startIndex,startIndex)
        pfr = self.addRewardNameToPFR(pfr,modRewName)
        #do actions now
        actions = ['r','u','l','d','s']
        for act in actions:
            actName = act+nameSuffix
            if act == 'r':
                guard = modVarNames[0]+'<'+endIndexC
                consq = self.stateString(modVarNames[0],modVarNames[0]+'+1')
                rew = 1.0
            elif act == 'u':
                guard = modVarNames[1]+'<'+endIndexC
                consq = self.stateString(modVarNames[1],modVarNames[1]+'+1')
                rew = 1.0
            elif act == 'l':
                guard = modVarNames[0]+'>'+startIndexC
                consq = self.stateString(modVarNames[0],modVarNames[0]+'-1')
                rew = 1.0
            elif act == 'd':
                guard = modVarNames[1]+'>'+startIndexC
                consq = self.stateString(modVarNames[1],modVarNames[1]+'-1')
                rew = 1.0
            else:
                continue
            rewDict = {modRewName:rew}
            guard = '('+guard+')'
            pa = gmod.createAction(actName,copy.deepcopy(rewDict),guard,consq,probValC)
            gmod.actions.append(copy.deepcopy(pa))        
        #end actions
        pfr.modVars.append(gmod)
        return [pfr,gmod]
 
    def createXYModule(self):
        ...
        probVal = 1.0
        probValC = 'p'
        #lets add the start and end stuff
        if startIndexC not in pfr.constants:
            pfr.addConstant(startIndexC,'int',str(startIndex))
        if endIndexC not in pfr.constants:
            pfr.addConstant(endIndexC,'int',str(endIndex))
        if probValC not in pfr.constants:
            pfr.addConstant(probValC,'double',str(probVal))

        xVarName = 'x'
        yVarName = 'y'
        modRewName = '"time"' 
        numModules = 2
        for nm in range(numModules):
            nameSuffix = str(nm)
            modName = 'R'+nameSuffix
            pfr = self.checkModInPFR(pfr,modName)

            modVarNames = [xVarName+nameSuffix, yVarName+nameSuffix]
            [pfr,gmod]=self.createSingleXYModule(pfr,modName,modVarNames,startIndex,endIndex,modRewName,startIndexC,endIndexC,probValC,nameSuffix)
        fn = '/home/fatma/Data/PhD/code/prism_ws/prism-svn/prism/tests/wkspace/'+'xytest.prism'
        #now we need to create the scheduler
        modName = 'sched'
        modVarName = 'turn'
        
        lines = pfr.createFileLinesToWrite()
        pfr.writeLinesToFile(lines,fn)
