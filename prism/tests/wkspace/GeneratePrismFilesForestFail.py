...
class GeneratePrismFile(object):
    ...
    failstateRewVar = 'failstateRew'
    foreststateRewVar = 'foreststateRew'
    ...
    def addFailStateRewVarConstant(self,pfr):
        if self.failstateRewVar not in pfr.constants:
            pfr.addConstant(self.failstateRewVar,'double',str(1000.0))
        return pfr
    
    def addForestStateRewVarConstant(self,pfr):
        if self.foreststateRewVar not in pfr.constants:
            pfr.addConstant(self.foreststateRewVar,'double',str(10.0))
        return pfr
    ...
    def createActionsFromLists(self,pfr,mod,xside,yside,varname,initStates,blockedStates,failstates,connectedStates,avoidStates,doFour):
        smap = {}
        scount = len(smap)
        #doFour = True
        #default = 8
        for xy in connectedStates:
            ...
            for xyp in combs:
                ...
                isAvoidState = xyp in avoidStates
                rew = 0 
                
                if isFailState:
                    rew = self.failstateRewVar
                
                elif isAvoidState:
                    rew =self.foreststateRewVar
                else:
                    rew = math.sqrt(((xp-x)*(xp-x))+((yp-y)*(yp-y)))
                #print rew
                #print xyp
    ...
    def generateModuleLinesFromLists(self,initStates,blockedStates,goalStates,avoidStates,failstates,connectedStates,doorStates,xside,yside,pfr,modName,doFour):
        ...
        pfr = self.addFailStateRewVarConstant(pfr)
        pfr = self.addForestStateRewVarConstant(pfr)
        ...
        if pfr.modVars is None:
            pfr.modVars = []
        [gmod,pfr,smap] = self.createActionsFromLists(pfr,gmod,xside,yside,varname,initStates,blockedStates,failstates,connectedStates,avoidStates,doFour)
    ...
    def generateFromGUIGrid(self,initStates,blockedStates,goalStates,avoidStates,failStates,connectedStates,doorStates,xside,yside,fn,doFour):
        ...
        pfr.writeGoalStatesAvoid(newGoalStates,[],varname,fn+'.props')
        pfr.writeGoalStatesAvoidReward(newGoalStates,[],varname,fn+'_rew.props',pfr.rewardNames[0])
        pfr.writeGoalStatesSplitAvoidLabels(newGoalStates,[],varname,fn+'.prop')
