...
class Cell():
    ...
    FILL_COLORS= {GuiState.OBSTACLES:'#000000',GuiState.EMPTY:'#FFFFFF',GuiState.INITLOCS:'#0000FF',GuiState.GOALS:'#00FF00',GuiState.AVOIDSTATES:'#FF0000',GuiState.FAILSTATES:'#888888',GuiState.DOORS:'#923A00'}
...
class GridGuiRandomiseDialog(tkSimpleDialog.Dialog):
    ...
    def apply(self):
        ...
        print doGS
        print doIS
        self.result = (numRandomFiles,doFS,doGS,doIS)
...
class GridGui(object):
    ...
    def allDone(self):
        ...
        smap=gfr.generateFromGUIGrid(initStates,blockedStates,goalStates,avoidStates,failStates,connectedStates,doorPairs,self.xside,self.yside,self.fn+"_fsgen0_",doFour)
        for ij in smap:
            i = ij[0]
            j = ij[1]
            lab = smap[ij]
        ...
        if self.randomiseFS or self.randomiseGoals or self.randomiseInitLocs:
            ...
            for i in range(len(fsLists)):
                ...
                fn = self.fn+'fs'+str(len(fsLists[i]))+"_fsgen"+str(fscount)+"_r"+str(iscount)+"_t"+str(gscount)
