    ...
    for state in allBounds:
        ...
        if mp:
            ...
            if mr:
                ...
                m = m and mc
                if mc:
                    mup = monotonic(sb.upperP)
                    m = m and mup
                    if mup:
                        mlp = monotonic(sb.lowerP)
                        m = m and mlp
                        if mlp:
                            mlr=monotonic(sb.lowerR)
                            m = m and mlr
                            if mlr:
                                mur=monotonic(sb.upperR)
                                m = m and mur
                                if mur:
                                    mlc=monotonic(sb.lowerC)
                                    m = m and mlc
                                    if mlc:
                                        muc=monotonic(sb.upperC)
                                        m = m and muc
                                        if not muc:
...
def plot_updates():
    fns = []
    while(True):            
        files=list_files_by_date()
        #print (files)
        #print (fns)
        #print len(files)
        #print len(fns)
        #list_files()
        if len(files) > len(fns):
            fns = files
            print(fns)
            #fns=[]    
            for fn in fns:
