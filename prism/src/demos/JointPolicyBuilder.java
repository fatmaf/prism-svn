...
//			jointStateQueue.add(null); // add the joint state to the queue
			while (!jointStateQueue.isEmpty()) {
				...
				// switch
				if (getNewTaskAllocation(statesDiscovered,robotStatesInSeqTeamMDP)) //get new task allocation 
				...
				HashMap<Integer, ArrayList<Entry<Integer, Integer>>> mostProbableTaskAllocationStateValues = 
						getStateIndexValuesForTaskAllocationForAllRobots(statesDiscovered,mdp.getStatesList(),mdp.getVarList());
				
				HashMap<Integer, ArrayList<Entry<Integer, Integer>>> stateValuesBeforeTaskAllocation = 
						getStateIndexValuesBeforeTaskAllocationForAllRobots(statesDiscovered,mdp.getStatesList(),mdp.getVarList());
				
				int[] modifiedRobotStatesInSeqTeamMDP = modifyRobotStatesToReflectExpectedTaskCompletion(
						robotStatesInSeqTeamMDP, mostProbableTaskAllocationStateValues,mdp.getStatesList(),0); //FIXME: hardcoding first robot fix this please

				 Entry<String,ArrayList<Entry<int[],Double>>> actionAndCombinations= getActionAndSuccStatesAllRobots(strat, modifiedRobotStatesInSeqTeamMDP,mdp);
...
//					{
						...
						int[] newSuccStatesForJointState = modifyRobotStatesToUndoExpectedTaskCompletion(
								modifiedRobotSuccStatesInSeqTeamMDP, stateValuesBeforeTaskAllocation,mdp.getStatesList(),0);
						State succJointState = createJointState(newSuccStatesForJointState, mdp.getStatesList(), false, 0, mdp.getVarList());
...
//		}
	...
	private void addTranstionToMDP(MDPSimple mdp,State parentStates, ArrayList<State> states, ArrayList<Double> probs, String action,
			double norm) {
...
//			}
	...
	//right now if any state is not in the ones we found earlier, we redo task allocation 
	//a smarter way would be to redo task allocation only for those states (hence robots) that are not on the path 
	//TODO: what I said above
	private boolean getNewTaskAllocation(ArrayList<ArrayList<StateExtended>> statesDiscovered, int[] robotStatesInSeqTeamMDP) {
...
//		mainLog.println("getNewTaskAllocation() not implemented");
	...
	private int[] modifyRobotStatesToReflectExpectedTaskCompletion(int[] robotStatesInSeqTeamMDP,
		...
		for(int i = 0; i<robotStatesInSeqTeamMDP.length; i++)
		{
			...
			ArrayList<Entry<Integer, Integer>> indicesToChange = mostProbableTaskAllocationStateValues.get(prevRobot);
	...
	protected HashMap<Integer, ArrayList<Entry<Integer, Integer>>> getStateIndexValuesForTaskAllocationForAllRobots(ArrayList<ArrayList<StateExtended>> statesDiscovered, List<State> statesList, VarList varList) {
		...
		HashMap<Integer, ArrayList<Entry<Integer, Integer>>> changedStatesForAllRobots = new HashMap<Integer, ArrayList<Entry<Integer, Integer>>>();
			for(int i = 0; i<statesDiscovered.size(); i++)
			{
				...
				{
					if (changedStates[j]==1 && !this.sharedStatesNamesList.contains(varList.getName(j)) && !this.isolatedStatesNamesList.contains(varList.getName(j)))
					{
	...
	protected HashMap<Integer, ArrayList<Entry<Integer, Integer>>> getStateIndexValuesBeforeTaskAllocationForAllRobots(ArrayList<ArrayList<StateExtended>> statesDiscovered, List<State> statesList, VarList varList) {
	...
	protected ArrayList<ArrayList<StateExtended>> getTaskAllocationForAllRobots(MDStrategyArray strat, MDPSimple mdp, int[] initialStates) {
		ArrayList<ArrayList<StateExtended>> statesDiscovered = new ArrayList<ArrayList<StateExtended>>();
	...
	return actionChoices;
	 ...
	 protected  Entry<String,ArrayList<Entry<int[],Double>>> getActionAndSuccStatesAllRobots(MDStrategyArray strat, int[] states,MDPSimple mdp) {
		 ...
		 //for each action choice we need to get the successorstates 
		  ...
		  for(int i = 0; i<states.length; i++)
		  {
			  ...
			  Iterator<Entry<Integer, Double>> tranIter = getTranIter(actionChoices.get(states[i]).getValue(),states[i],mdp);
...
//			  State succJointState= createJointState(currentSuccStates,mdp.getStatesList(),false,0,null);
...
//				robotStates.add(robotState);
		...
		else
		{
			...
			ArrayList<Object[]> robotStateElements = StatesHelper.getDAandSharedStatesFromState(robotState, varlist, sharedStatesNamesList, numTasks);
			...
			do {
				...
				ArrayList<Object[]> nextRobotStateElements = StatesHelper.getDAandSharedStatesFromState(nextRobotState, varlist, sharedStatesNamesList, numTasks);
				Object[] nextRobotDAs = nextRobotStateElements.get(0); 
				Object[] nextRobotSS = nextRobotStateElements.get(1);
				for(int j = 0; j<das.length; j++)
				{
					if((int)stateToRet.varValues[j]!= (int)nextRobotDAs[j] && (int)nextRobotDAs[j]!= daInitialStates.get(j))
					{
				...
				for(int j = 0; j<ss.length; j++)
				{
					...
					if(((int)stateToRet.varValues[j+das.length])!=(int)nextRobotSS[j] && (int)nextRobotSS[j]!=sharedVarsInitialStates.get(this.sharedStatesNamesList.get(j)))
					{
