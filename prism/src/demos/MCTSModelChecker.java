...
public class MCTSModelChecker 
{
	...
	private PrismComponent prismcomponent; 
	...
	private Result checkExpressionProb(ExpressionProb expr) throws PrismException
	{
		...
		LTLModelChecker ltlMC = new LTLModelChecker(this.prismcomponent);
		...
		for (int i = acc.nextSetBit(0); i >= 0; i = acc.nextSetBit(i + 1)) {
			...
			uct = new UCT(this.mainLog, prodModelGen, prodModelGen.getInitialState(), distsToAcc, sinkStates, depth, nSamples, useDistCost, null); //TODO iterative deepening; tweak exploration param, # samples, depth
			uctRes = uct.search();
...
*/		
	...
	public MDPSimple buildMDP(demos.UCT.UCTNode node, BitSet terminalStates, MDPSimple mdp) throws PrismException{
		int i, currentStateIndex, succStateIndex, nSuccs;
		double prob;
		Distribution distr;
		String action;
		demos.UCT.UCTNode currentNode, currentSucc, bestSucc, succs[];
		Deque<demos.UCT.UCTNode> nodeQueue = new ArrayDeque<demos.UCT.UCTNode>();
...
//			System.out.println(currentNode.getStateCost());
	...
	public MDPSimple buildMDP(demos.UCT.UCTNode node, ModelGenerator modelGen, BitSet terminalStates) throws PrismException{
...
/*	private ModelCheckerResult checkMaxProg(MDP policyUCT) {
	...
	*/
	private StateValues checkUCTResult(demos.UCT.UCTNode result, Expression expr, DA<BitSet,? extends AcceptanceOmega> da, ProductModelGenerator prodModelGen, String modelName) throws PrismException {
