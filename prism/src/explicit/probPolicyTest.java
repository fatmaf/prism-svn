...
class teamAutomatonWithoutSwitches {
	...
	public teamAutomatonWithoutSwitches(int numrobots, BitSet accstatesf, BitSet statestoavoid,
			MDPSimple teamAutomatonNoSwitches, BitSet[] switchStates, MDPRewardsSimple rewards) {
		numRobots = numrobots;
...
public class probPolicyTest {
	...
	public ArrayList<Map.Entry<policyState, ArrayList<ArrayList<ArrayList<policyState>>>>> allPoliciesList; // includes
																											// all the
																											// policies,
																											// the same
																											// information
																											...
																											// (with
																											// time)
	public ArrayList<ArrayList<ArrayList<policyState>>> currentPolicy; // the policy that is currently being used /
																		// explored
	...
	private ArrayList<Map.Entry<tempState, Integer>> combinedPolicyStatesMap; // contains the mapping from the original
																				// team automaton to this policy mdp
	// stuff related to the mdp bit, maybe store this somewhere else but its helpful
	// here too
	// public int numRobots; //the number of robots does not change so its okay to
	// store it
	// public BitSet finalAccStates; //the accepting states dont change so its okay
	// to store them here
	// public int numStates; //the number of states in the initial MDP so no one has
	// to remember it
	// public stateInfoFromStatesList allStates;
	// public BitSet statesToAvoid;
	...
	public ArrayList<ArrayList<Entry<policyState, ArrayList<Entry<Integer, Integer>>>>> getChangedIndicesAcrossPolicyAtTime(
	...
	public ArrayList<ArrayList<Entry<policyState, ArrayList<Entry<Integer, Integer>>>>> getChangedIndicesAcrossPolicyAtTime(
			ArrayList<ArrayList<ArrayList<policyState>>> policy, policyState currRobotState) {
		...
		for (int r = 0; r < tAutomaton.numRobots; r++) {
			...
			// what i'm trying to do is make sure that changes from previous bits are
			// reflected. However it is important to (omg you're done with this however
			// stuff okay)
			// remember that if we're going round, then those changes need to be preserved
			// for robots > currRobotNum too.
			{
			...
			// for each state at this time get the changed values from this state and the
			// state at the beginning
			for (int s = 0; s < currStates.size(); s++) {
				currState = currStates.get(s);
				ArrayList<Entry<Integer, Integer>> changedIndicesForS2 = tAutomaton.allStates
						.compareDAStatesOnly(stateAtBeginning.state, currState.state, true);

				// //compare with previous state.
				if (r < currRobotNum) {
					ArrayList<Entry<Integer, Integer>> changedIndicesBetweenRAndCurrRobotState = tAutomaton.allStates
							.compareDAStatesOnly(currState.state, nextRobotBeginning.state, false);
					changedIndicesForS2 = combineTwoArrayLists(changedIndicesForS2,
							changedIndicesBetweenRAndCurrRobotState);
				}
				// but we also want to remember that the changes for currRobotNum override all
				// the others
				// something we could fix elsewhere ?
				// TODO: above
				changedStates.or(tAutomaton.allStates.markIndicesInBitSetFromArrayList(changedIndicesForS2)); // so just
																												// saving
																												// all
																												// the
																												// states
			...
			// great now we want to make sure we have values for each changed da element for
			// each robot state we've compared
			// so we want to remember the da states that were changed and their values even
			// if they didnt change for this particular state but changed for some other
			// state
			// honestly at this point i have no idea why i'm doing this so i'm just going to
			// go with it
			...
			for (int ci = 0; ci < changedIndicesForStates.size(); ci++) {
				...
				Entry<policyState, ArrayList<Entry<Integer, Integer>>> changedIndicesForStateEntry = changedIndicesForStates
						.get(ci);
				...
				if (totalChangedIndices != changedIndicesForStateSize) {
					// okay so just get the ones that you dont have values for and then add values
					// for those
	...
	public void addLinkInCombinedPolicyMDP(ArrayList<int[]> states, ArrayList<Double> probs, int[] parentState,
			Object action, String additionalText) {
		...
		// just to check if I have multiple branches because I'm going over the same
		// stuff again
		int numChoices = combinedPolicyMDP.getNumChoices(combinedPolicyStatesMap.get(indexPS).getValue());
		if (action == null)
			combinedPolicyMDP.addActionLabelledChoice(combinedPolicyStatesMap.get(indexPS).getValue(), distr,
					additionalText + "_" + actionText + "." + numChoices);
		else
			combinedPolicyMDP.addActionLabelledChoice(combinedPolicyStatesMap.get(indexPS).getValue(), distr,
	...
	public void addAllPoliciesListToCombinedPolicyMDP() {
		...
		for (int polNo = 0; polNo < allPoliciesList.size(); polNo++) {
			Entry<policyState, ArrayList<ArrayList<ArrayList<policyState>>>> currentPolPair = allPoliciesList
					.get(polNo);
	...
	public BitSet addStateMDP(MDPSimple mdp, int[] mdpMap, int state,BitSet accStates) 
		{
			return addStateMDP(mdp,mdpMap,tAutomaton.allStates.states,BADVALUE,state,accStates);
		}
	public BitSet addStateMDP(MDPSimple mdp, int[] mdpMap, List<State> states,
			int stateNotAdded,int state,BitSet accStates) {
		if (mdpMap[state] == stateNotAdded) {
			// add to states list
			mdpMap[state] = mdp.getNumStates();
			mdp.statesList.add(states.get(state)); // should work
			mdp.addState();
			if (tAutomaton.finalAccStates.get(state))
			{
	...
	public void addLinkInPolicyMDP(int[] states, double[] probs, int parentState, Object action, boolean normalize,
		...
		if (normalizer != 0) {
			...
			if (action == null)
				policyMDP.addActionLabelledChoice(policyStatesMap[parentState], distr,
						additionalText + "_" + actionText + "." + numChoices);
			else
				policyMDP.addActionLabelledChoice(policyStatesMap[parentState], distr,
						additionalText + "_" + action.toString() + "." + numChoices);
	...
	public void addLinkInPolicyMDP(int state, double prob, int parentState, Object action, String additionalText) {
		...
		if (action == null)
			...
			policyMDP.addActionLabelledChoice(policyStatesMap[parentState], distr,
					additionalText + "_" + action.toString() + ".");
		// ^adding +numChoices to the above line caused problems, I have no idea why,
		// probably need to ask dave, same state same stuff ?
	...
	public BitSet addLinkInMDP(MDPSimple mdp,int[] mdpMap,List<State> statesList,
			ArrayList<Integer> states, 
			ArrayList<Double> probs, int parentState, Object action,
			String additionalText,BitSet accStates) {
		accStates = addStateMDP(mdp,mdpMap,statesList,BADVALUE,parentState,accStates);
		Distribution distr = new Distribution();
	...
	public probPolicyTest(int numrobots, BitSet accstatesf, BitSet statestoavoid, MDPSimple teamAutomatonNoSwitches,
			BitSet[] switchStates, PrismLog log, MDPRewardsSimple rewards) {

		stateNotAddedToPolicyVal = -1;
		// numRobots = numrobots;
		// finalAccStates = (BitSet)accstatesf.clone();
		// statesToAvoid = (BitSet)statestoavoid.clone();
		// numStates = teamAutomatonNoSwitches.numStates; //they should be the same with
		// or without switches
		// allStates = new stateInfoFromStatesList(teamAutomatonNoSwitches); //should be
		// the same with or without switches
		mainLog = log;
		printPolicy = false;
		printHighlights = true;
		// teamAutomatonNoSwitches = copyMDPWithVarList(teamAutomatonNoSwitches); //so
		// it doesnt have to be passed
		tAutomaton = new teamAutomatonWithoutSwitches(numrobots, accstatesf, statestoavoid, teamAutomatonNoSwitches,
				switchStates, rewards);
		...
		jointPolicy= new MMDPSimple(numrobots);
...

	{
		...
		while (initialStatesIter.hasNext()) {
			...
			policyState testState = new policyState(currState);// new policyState(rnum, timecount, state, parent_state);
																// //so the timecount is always 0
			testState.state = state;
			testState.rnum = rnum;

			ArrayList<Integer> statesDiscovered = unfoldPolicyForState(sumprod, strat, testState, succState,
					allRobotInitStates);
	...
	public ArrayList<Integer> unfoldPolicyForState(MDPSimple sumprod, Strategy strat, policyState rts,
		...
		Stack<policyState> statesStack = new Stack<policyState>(); // to store all the states we need to explore, so dfs
		...
		policyComputedFor[rts.rnum].set(rts.state); // say that we've computed the policy for this state now //we dont
													// know if we'll need to add a link later
										...
										// so on
		{
			...
			while (currentPolicy.get(currState.rnum).size() < ((timecount - rts.time) + 1)) {// +1 because we're adding
																								// successors
			...
			if (!discoveredStates.contains(currState)
					...
					& !tAutomaton.finalAccStates.get(currState.state)) // if it is not an accepting state and not
																		// discovered explore
			{
				// add it if it is a stuck state
				if (action == null || "*".equals(action)) {
					// if this state has no next actions and it is not a final state then we need to
					// add it to the stuck states Queue
					// you cant add this state if the policy has already been computed for this
					// state
					...
					if (!allRobotsHaveFailed(allRobotInitStates, currState)) {
						...
						if (policyComputedForList.contains(currState)
								&& arrayListContainsArray(policyComputedForListAllRobots, allRobotInitStates)) {
							// then you can add its successor states [basically get the next robot and add
							// that state (like adding a switch)
							...
							if (nextSuccState != -1) {
								...
								if (nextState.cumulativeProb > probThreshold) {
									...
									nextState.associatedPolicyID = allPoliciesList.size(); // this works on the
																							// assumption that the
																							// current policy is empty ^

									String textToAdd = currState.state + "_" + currState.pstate + "_"
											+ rts.associatedPolicyID;// "abc";//currState.state+"_"+currState.associatedStartState;

									addLinkInPolicyMDP(nextState.state, 1.0, currState.state,
											"switch_er_" + currState.rnum + "_" + nextState.rnum, textToAdd);
									// ^ caused the edge lines to disappear in the policy mdp, I'm not entirely sure
									// why, so I removed the number in this function
							...
							// TODO: this should only be the case for policies that have been expanded and
							// are being expanded for the first time.

							else {
								mainLog.println("This state is interesting " + currState.toString());
								// throw new PrismException("Something unexpected happened while unrolling the
								// policy for "+rts.toString()+" at "+currState.toString());
							}
						} else {
						...
						// once we've discovered a state we do not add it to the stuck state (cuz we
						// added it once before)
						// does this need fixing ?
						// if the state and associated state are the same that means we're trying to
						// compute it again
						...
						if (printHighlights)
							mainLog.println(
									"All robots have failed here " + allRobotString + " " + currState.toString());
					}
				}
					...
					if (action.equals(sumprod.getAction(currState.state, c))) {
						...
						while (iter.hasNext()) {
							...
							if (action.toString().contains(switchInFailureAction))
								succState = new policyState(tAutomaton.allStates.getRobotNum(stateProbPair.getKey()),
										currState.time, stateProbPair.getKey(), currState.state, null, rts.state); // just
																													// go
																													// back
																													// one
																													// time
								...
								succState = new policyState(tAutomaton.allStates.getRobotNum(stateProbPair.getKey()),
										rts.time, stateProbPair.getKey(), currState.state, null, rts.state); // same as
																												// the
																												// initial
																												// states
																												// time
						...
						addLinkInPolicyMDP(states, probs, currState.state, action,
								currState.state + "_" + rts.state + "_" + rts.associatedStartState);
						accStates[currState.rnum] = addLinkInMDP(mdps[currState.rnum],mdpMaps[currState.rnum],
								sumprod.getStatesList(),
								states, probs, currState.state, action,
								"",accStates[currState.rnum]);
						if(mdps[currState.rnum].getNumInitialStates()==0)
							mdps[currState.rnum].addInitialState(0);
						// addLinkInPolicyMDP(states,probs, currState.state, action);
						break;
	...
	private void checkCurrentPolicy(int[] robotInitStates, policyState state) {
		...
		for (int r = 0; r < robotInitStates.length; r++) {
			if (currentPolicy.get(r).size() == 0) // if the size of the policy for this robot is 0 we need to add an
													// init state
			{
	...
	private boolean checkMDPs(MDPSimple mdps[], int[][] mdpMaps,List<State> states,int[] robotInitStates, policyState state,BitSet[] accStates) {
		boolean res = true;
		for (int r = 0; r < tAutomaton.numRobots; r++) {
	...
	private void createPolicyFromRobotInitStates(int[] robotInitStates, policyState state, boolean addAction) {
		...
		// there is no way to check this actually but meh //so TODO: galiyan -
		// muskurahat meri bolay kya socho naaaa :P
		for (int r = 0; r < robotInitStates.length; r++) {
			policyState currState = createRobotState(robotInitStates[r], state);
	...
	// if policy for state found in all polices change it, otherwise let it be the
	// same
	...
	private ArrayList<ArrayList<ArrayList<policyState>>> getPolicyFromAllPoliciesForState(int state) {
		...
		for (int i = allPoliciesList.size() - 1; i >= 0; i--) {
			...
			if (currentState.getKey().state == state) { // possible errors because of this matching - cuz you dont care
														// about the pstate or associatedstate and this is important
				res = deepCopyPolicy(currentState.getValue());
				break; // this does not get the latest ? do I care about this ? this will change things
						// so maybe reverse the order
			}
		}
		return res;
	...
	private ArrayList<ArrayList<ArrayList<policyState>>> getPolicyFromAllPoliciesIDForState(int polID) {
		...
		// if (currentState.getKey().state == state) { //possible errors because of this
		// matching - cuz you dont care about the pstate or associatedstate and this is
		// important
		res = deepCopyPolicy(currentState.getValue());
		// break; // this does not get the latest ? do I care about this ? this will
		// change things so maybe reverse the order
		// }
	...
	private ArrayList<ArrayList<ArrayList<policyState>>> deepCopyPolicy(
			ArrayList<ArrayList<ArrayList<policyState>>> tocopy) {
